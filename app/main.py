...
from app.ocr_reader import extract_mrz_lines
...
async def read_mrz(file: UploadFile = File(...)):
    ...
    try:
        ocr_result = extract_mrz_lines(image_bytes)
    ...
    # Ưu tiên tìm MRZ trong vùng crop đáy ảnh trước (ít nhiễu hơn), fallback sang toàn ảnh
    all_candidate_lines = ocr_result["lines_bottom_crop"] + ocr_result["lines_full_image"]
    parsed = find_and_parse_mrz(all_candidate_lines)

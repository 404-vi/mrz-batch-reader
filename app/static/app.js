...
const CONCURRENCY = 1; // số ảnh xử lý song song tối đa
...
function statusPillHtml(item) {
  switch (item.status) {
    ...
    case "success":
      return `<span class="status-pill status-success">${ICON_CHECK}Thành công</span>`;
    case "warning":
      return `<span class="status-pill status-warning">${ICON_WARNING}Cảnh báo</span>`;
    case "error":
      return `<span class="status-pill status-error">${ICON_ERROR}Lỗi</span>`;
    default:
      return "";
  }
}
...
function render() {
  ...
  // Dải thống kê (luôn cập nhật, kể cả khi danh sách rỗng)
  ...
  statAccuracy.textContent = anyProcessed
    ? `${Math.round(((successCount + warningCount) / (successCount + warningCount + errorCount)) * 100)}%`
    : "—";
